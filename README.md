...
- **无单例**：`Server` 拥有自己的 `ThreadPool` 与 `Dispatcher`，可同时跑多个实例、可嵌入、可测试
...
│   ├── thread_pool.h           # ThreadPool（一线程一 io_context）
│   ├── frame.h                 # 帧编解码 seam
│   └── msg_node.h              # 收发消息节点
├── src/                        # 库实现
├── examples/
...
void on_hello(std::shared_ptr<mts::Session> session, short msg_id, std::string body)
{
